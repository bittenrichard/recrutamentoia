...
const DayView = ({ currentDate, events, vagaColorMap, onSelectEvent }) => {
    ...
    const dailyEvents = useMemo(() => {
        return events.filter(event => format(event.start, 'yyyy-MM-dd') === format(currentDate, 'yyyy-MM-dd')).sort((a, b) => a.start.getTime() - b.start.getTime()); // Ordena por hora
    }, [events, currentDate]);
    return (
      ...
      <div className="flex-grow overflow-y-auto hide-scrollbar">
        ...
        <div className="grid grid-cols-2 min-h-full">
          ...
          <div className="col-span-1 relative"> 
            ...
            {hoursOfDay.map((hour, hourIndex) => (<div key={hourIndex} className="h-16 border-b border-dashed border-gray-100 pr-2"></div>))}
            ...
            return (<div key={event.resource.id} className="absolute left-1 right-1 p-2 rounded-md text-white overflow-hidden shadow-md cursor-pointer" style={{
                    top: `${topOffset}px`,
                    height: `${height}px`,
