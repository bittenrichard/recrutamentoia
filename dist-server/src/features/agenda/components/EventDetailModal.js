...
const EventDetailModal = ({ event, onClose }) => {
    ...
    const googleEventLink = resource.google_event_link; // Acessando o link do evento, se existir
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg transform transition-all duration-300 scale-95 opacity-0 animate-scale-in">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3"> {/* Contêiner para os botões de ação */}
            {googleEventLink && (<a href={googleEventLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-full gap-2 px-4 py-2 text-sm font-semibold rounded-md transition-colors bg-white border border-gray-300 text-gray-700 hover:bg-gray-100">
                <LinkIcon size={16}/>
                Ver no Google Calendar
              </a>)}

            {/* NOVO: Botão para download do currículo */}
            {candidateCurriculum ? (<a href={candidateCurriculum.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-full gap-2 px-4 py-2 text-sm font-semibold rounded-md transition-colors bg-white border border-gray-300 text-gray-700 hover:bg-gray-100">
                <Download size={16}/>
                Baixar Currículo
              </a>) : (<button disabled className="flex items-center justify-center w-full gap-2 px-4 py-2 text-sm font-semibold rounded-md bg-gray-100 text-gray-400 cursor-not-allowed" title="Currículo não disponível">
                <Download size={16}/>
