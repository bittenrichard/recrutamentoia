...
const ApprovedCandidatesModal = ({ candidates, onClose, isLoading }) => {
    const getScoreColor = (score) => {
        if (score >= 95)
            return 'text-green-600';
        return 'text-blue-600';
    };
    return (<div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4 transition-opacity duration-300">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] flex flex-col transform transition-all duration-300 scale-95 opacity-0 animate-scale-in">
        ...
        <div className="p-6 overflow-y-auto">
          {isLoading ? (<p className="text-center py-8">Carregando candidatos...</p>) : candidates.length === 0 ? (<p className="text-center text-gray-500 py-8">Nenhum candidato com score acima de 90% encontrado.</p>) : (<table className="w-full text-left">
              <thead>
