...
export const useDashboardStats = (jobs, candidates) => {
    const stats = useMemo(() => {
        ...
        const activeCandidates = candidates.filter(candidate => candidate.vaga && candidate.vaga.some(v => activeJobIds.has(v.id)));
        ...
        const approvedCandidates = activeCandidates.filter((c) => c.score && Number(c.score) >= 90).length;
        let averageScore = 0;
