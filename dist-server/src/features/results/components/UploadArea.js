...
const UploadArea = ({ onFilesSelected, isUploading = false, }) => {
    ...
    return (<div className={`relative bg-white p-8 rounded-lg shadow-sm border-2 border-dashed text-center mb-8 transition-colors ${isUploading
            ? 'border-indigo-300 bg-indigo-50 cursor-wait'
            : 'border-gray-300 hover:border-indigo-400'}`} onDrop={handleDrop} onDragOver={handleDragOver}>
      {isUploading ? (<div className="w-full max-w-md mx-auto">
          ...
          <label className="mt-4 inline-block">
            <input type="file" multiple accept=".pdf,.docx,.doc" onChange={handleFileSelect} className="hidden" disabled={isUploading}/>
