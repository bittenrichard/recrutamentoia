...
interface WeekViewProps {
  currentDate: Date; // A data que a semana está centralizada
  events: CalendarEvent[]; // Todos os eventos carregados
  onDayClick: (date: Date) => void; // Para clique em um dia específico (pode abrir sidebar ou day view)
...
const WeekView: React.FC<WeekViewProps> = ({ currentDate, events, onDayClick, selectedDayForSidebar, vagaColorMap, onSelectEvent }) => {
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="grid grid-cols-8 flex-shrink-0 border-b border-gray-200 bg-gray-50">
        ...
        {daysOfWeek.map((day, index) => (
          <div 
            key={index} 
            className={`col-span-1 calendar-day-header ${isSameDay(day, selectedDayForSidebar) ? 'is-selected' : ''} ${isToday(day) ? 'is-today' : ''}`}
            onClick={() => onDayClick(day)} // Permite selecionar o dia clicando no cabeçalho
          >
        ...
        <div className="grid grid-cols-8 min-h-full">
          ...
          {daysOfWeek.map((day, dayIndex) => {
            ...
            return (
              <div 
                key={dayIndex} 
                className={`col-span-1 relative border-r border-gray-100 `}
              >
                {/* Linhas de tempo para cada hora */}
                {hoursOfDay.map((hour, hourIndex) => (
                  <div key={hourIndex} className="h-16 border-b border-dashed border-gray-100"></div>
                ))}
                ...
                {dayEvents.map((event) => {
                  ...
                  return (
                    <div
                      ...
                      className="absolute left-1 right-1 p-1 rounded-md text-white overflow-hidden shadow-md cursor-pointer"
                      style={{
                        top: `${topOffset}px`,
                        height: `${height}px`,
