...
interface CustomEventProps {
  event: CalendarEvent;
  [key: string]: any; // Permite outras props desconhecidas do RBC, como 'view' e 'style'
}

const CustomEvent: React.FC<CustomEventProps> = ({ event, view, ...props }) => {
  ...
  if (isMonthView) {
    ...
    return (
      <div 
        ...
        title={`${event.title} (${startTime} - ${endTime}) com ${candidate?.nome || 'N/A'}`} // Tooltip completo
      >
    ...
    return (
      <div 
        className="flex flex-col h-full p-2 rounded-md text-white overflow-hidden" // Aumentamos um pouco o padding
        style={{ backgroundColor }} 
      >
